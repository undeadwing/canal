 ...
 */
public class MQFlatFlatMessageUtils {
    private static Logger logger = LoggerFactory.getLogger(MQFlatFlatMessageUtils.class);
     ...
     */
    public static List<FlatFlatMessage> messageConverter(Message message) {
        try {
            if (message == null) {
                return null;
            }

            List<FlatFlatMessage> flatFlatMessages = new ArrayList<>();
...
//                    List<Map<String, String>> old = new ArrayList<>();
                    ...
                    for (CanalEntry.RowData rowData : rowChange.getRowDatasList()) {
                        ...
                        FlatFlatMessage flatFlatMessage = new FlatFlatMessage(message.getId());
                        flatFlatMessages.add(flatFlatMessage);
                        flatFlatMessage.setDatabase(entry.getHeader().getSchemaName());
                        flatFlatMessage.setTable(entry.getHeader().getTableName());
                        flatFlatMessage.setIsDdl(rowChange.getIsDdl());
                        flatFlatMessage.setOptType(eventType.toString());
                        flatFlatMessage.setEs(entry.getHeader().getExecuteTime());
                        flatFlatMessage.setTs(System.currentTimeMillis());
                        flatFlatMessage.setSql(rowChange.getSql());
                        flatFlatMessage.setLogFileName(entry.getHeader().getLogfileName());
                        flatFlatMessage.setLogfileOffset(entry.getHeader().getLogfileOffset());
                        ...
                        for (CanalEntry.Column column : columns) {
                            if ( column.getIsKey()) {
                                flatFlatMessage.addPkName(column.getName());
                            }
...
//                            data.add(row);
                        ...
                        if (eventType == CanalEntry.EventType.UPDATE) {
                            ...
                            // update操作将记录修改前的值
                            if (!rowOld.isEmpty()) {
                                flatFlatMessage.setOld(rowOld);
//                                old.add(rowOld);
                            }
                        }
                        //暂时用不到字段类型，这里先注释掉
//                        if (!sqlType.isEmpty()) {
//                            flatFlatMessage.setSqlType(sqlType);
//                        }
                        if (!mysqlType.isEmpty()) {
                            flatFlatMessage.setMysqlType(mysqlType);
                        }
                    }

//                    if (!data.isEmpty()) {
//                        flatFlatMessages.setData(data);
//                    }
//                    if (!old.isEmpty()) {
//                        flatFlatMessage.setOld(old);
//                    }
                }else{
                    logger.warn("ddl操作："+entry.getHeader().getSchemaName()+"_"+entry.getHeader().getTableName()+"_"+rowChange.getSql());
                    NoticeUtil.sendNotice(entry.getHeader().getSchemaName(),entry.getHeader().getTableName(),"ddl操作",rowChange.getSql());
                }
            }
            return flatFlatMessages;
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
